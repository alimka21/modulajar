...
import { getUsers, saveUser, updateUser, deleteUser, getSettings, saveSettings } from '../services/storageService';
import { swal, toast } from '../services/notificationService';
import { LogOut, Users, Settings, LayoutDashboard, Plus, Trash2, Edit2, CheckCircle, XCircle, Search, Mail, Lock, User as UserIcon, ShieldCheck, Loader2, X, ExternalLink, Activity, BarChart3, AtSign, Zap, GraduationCap } from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout, onGoToApp }) => {
  ...
  const handleAddUser = async (e: React.FormEvent) => {
      ...
      try {
        // Raw password is sent to saveUser, which calls Supabase signUp
        const user: User = {
            ...
            role: 'user', 
            status: 'active', // Admin added users are active
        ...
        swal.fire({
            title: 'Gagal!',
            text: error.message || "Email mungkin sudah terdaftar.",
            icon: 'error'
        });
      } finally {
                  ...
                  <div className="space-y-6 animate-fade-in max-w-[95%] mx-auto">
                      <div className="flex justify-between items-center">
                          ...
                          <div className="flex gap-2">
                            <button onClick={refreshData} className="bg-slate-100 hover:bg-slate-200 text-slate-600 px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition">
                            ...
                            <div className="bg-white p-6 rounded-xl border border-blue-100 shadow-lg mb-6 relative animate-fade-in-down">
                                ...
                                <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                    <div>
                                        ...
                                        <input required type="text" value={newUser.name} onChange={e => setNewUser({...newUser, name: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white focus:ring-1 focus:ring-blue-500 outline-none" placeholder="Nama User" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">Username</label>
                                        <input required type="text" value={newUser.username} onChange={e => setNewUser({...newUser, username: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white focus:ring-1 focus:ring-blue-500 outline-none" placeholder="Username" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">Email</label>
                                        <input required type="email" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white focus:ring-1 focus:ring-blue-500 outline-none" placeholder="email@contoh.com" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">Password</label>
                                        <input required type="text" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white focus:ring-1 focus:ring-blue-500 outline-none" placeholder="Min 6 karakter" />
                                    </div>
                  ...
                  <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg border border-slate-200 overflow-hidden animate-fade-in-up">
                      ...
                      <div className="p-6">
                          <form onSubmit={handleSaveEditUser} className="space-y-4">
                              ...
                              {/* Remove Password Edit for now - as it requires Admin API / RPC */}
